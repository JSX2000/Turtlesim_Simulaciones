...
def go_to_goal (xgoal, ygoal):
	...
	while(True):
		...
		if distance_turtle < 5:
			i = 2
			x_2 = x2
			y_2 = y2 - distance_turtle/2
			dx = x - x2
			dy = y - y2
			alfa = 2*math.pi - math.atan2(dy,dx)
			while distance_turtle < 5:
				x_c = x_2 + 0.5*distance_turtle*math.cos(alfa+i)
				y_c = y_2 + 0.5*distance_turtle*math.sin(alfa+i)
				orientate(x_c, y_c)
				i =+ 1
				distance_turtle = abs(math.sqrt(((x2-x)**2)+((y2-y)**2)))
